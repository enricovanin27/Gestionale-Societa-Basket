...
export default function CalendarioPage() {
  ...
  // Squadre assegnate al preparatore (solo quando si è in /prep)
  const { data: prepSquadre } = useQuery({
    queryKey: ['my-prep-squadre', profile?.id, societaId],
    enabled: !!profile?.id && !!societaId && actingAsPrep,
    staleTime: 5 * 60_000,
    queryFn: async () => {
  ...
  const { data: sessioniAtletica = [] } = useQuery({
    queryKey: ['prep-sessioni-calendar', societaId, weekStartStr, weekEndStr],
    enabled: !!societaId && (isAdmin || actingAsAllenatore || actingAsPrep) && !!weekStartStr && !!weekEndStr,
    staleTime: 60_000,
    queryFn: async () => {
      const { data } = await supabase
        .from('prep_sessioni')
  ...
  const myCoachSquadre = useMemo(() => {
    // Caso preparatore: usa le squadre assegnate in prep_squadre
    if (actingAsPrep) {
      if (prepSquadre === undefined) return null // ancora in caricamento
      if (prepSquadre.length > 0) return prepSquadre
      // fallback al profilo se non ci sono righe in prep_squadre
    ...
    return [profile?.squadra, profile?.squadra2, profile?.squadra3].filter(Boolean)
  }, [allenatoreRow, profile, actingAsPrep, prepSquadre])
      ...
      {calTab === 'settimana' && (
        isLoading ? (
          ...
          <VistaSettimanaleCompleta
            ...
            getSessioneAtletica={(isAdmin || actingAsAllenatore || actingAsPrep) ? getSessioneAtletica : null}
          />
        )
      )}
            ...
            saveMutation.mutate(formData, {
              onSuccess: async () => {
                if (prepSessionData?.squadra) {
                  ...
                  const { error: prepErr } = await supabase.from('prep_sessioni').insert({
                    societa_id:     societaId,
